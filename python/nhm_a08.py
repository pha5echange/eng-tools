...
for index in range(len(fileNames)):
	...
	for node in diEnGraph.nodes():
		...
		# open file 'genres/nodeV.txt' and read
		genrePath = os.path.join("genres", nodeV + '.txt')
		genreInput = open(genrePath, "r")
			
		for line in genreInput:
			vArtist, vArtistStart, vArtistEnd, vFamiliarity, vHotness, vMbid = line.split(",")
		...
		if nodeInDeg == 1:
			...
			for edge in inEdges:
				...
				totalEdgeWeight += inEdgeWeight		

			nodeNewArtists = (nodeTotalArtists - inEdgeWeight)
			newVcount = int(nodeNewArtists)
			Hnode = 0.0
