...
# Gets a maximum of 1000 artists per genre (Echonest limit) with `years_active' and `hotttnesss' data, and MusicBrainzID (for later use)
...
genreRequests = int(input ("Enter the maximum number of genres to trawl (total on Oct 14th 2015 was 1383): "))
...
for g in response_genre['genres']:
	...
	while results < 1000: 
		response_artist = en.get('artist/search', genre = g['name'], start = startIndex, sort = ['artist_start_year-asc'], bucket = ['years_active', 'hotttnesss', 'id:musicbrainz'], results = ['100'])
		...
		for a in response_artist['artists']:
			...
			if yaString == "[]":
				...
				genreArtistList.write (a['name'] + '^' + yaString[12:16] + '^' + yaString[26:30] + '^' + str(hotNess) + '^' + MbID + '\n')
