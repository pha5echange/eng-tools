...
# Writes EchoNest ID to facilitate MB mapping later
...
# DATA FORMAT - CSV/Txt - Headings: Artist Name, EchoNest ID, Start Year, End Year, Familiarity, Hotttnesss, MusicBrainz ID
	...
	while results < 1000: 
		...
		for a in response_artist['artists']:
			...
			if yaString == "[]":
				...
				genreArtistList.write (artistName + ',' + enId + ',' + yaString[12:16] + ',' + yaString[26:30] + ',' + str(famiLiar) + ',' + str(hotNess) + ',' + MbID + '\n')
				artistGenreWrite += 1
				artistGenreCount += 1
